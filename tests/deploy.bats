...
@test "asterisk lab skeleton files exist" {
  ...
  run test -x scripts/install-freepbx.sh
  [ "$status" -eq 0 ]
}
...
@test "FreePBX installer dry-run uses official Debian installer" {
  run bash -lc "GONNECT_FREEPBX_OS_RELEASE=tests/fixtures/debian12-os-release bash scripts/install-freepbx.sh --dry-run"
  [ "$status" -eq 0 ]
  [[ "$output" == *"Would run:"* ]]
  [[ "$output" == *"sng_freepbx_debian_install.sh"* ]]
  [[ "$output" == *"apt-get install -y --no-install-recommends ca-certificates curl"* ]]
}

@test "FreePBX installer rejects unsupported host OS by default" {
  run bash -lc "GONNECT_FREEPBX_OS_RELEASE=tests/fixtures/ubuntu-os-release bash scripts/install-freepbx.sh --dry-run"
  [ "$status" -eq 1 ]
  [[ "$output" == *"fresh Debian 12.x hosts"* ]]
}

@test "install wrapper exposes FreePBX mode" {
  run grep -F 'scripts/install-freepbx.sh' scripts/install.sh
  [ "$status" -eq 0 ]

  run grep -F 'sudo ./scripts/install.sh --freepbx' README.md
  [ "$status" -eq 0 ]
}
